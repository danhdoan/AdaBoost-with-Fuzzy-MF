 ...
 *      also uses Fuzzy Triangular MF with 1-Standard Deviation
...
public class Main {
    ...
    public final static String pathWeakClass = pathOutput + File.separator + "weakclass"
            + File.separator + "20180118_fuzzy_tri_std3_19.wk";
    ...
    public static void main(String[] args) {
        ...
        testingProcess(pathTest, pathTestInfo, 0.5, 0.5);

        //testWeakClass(pathTest, pathTestInfo);
        //testWeakClass(pathTrain, pathTrainInfo);
    }

    public static void testWeakClass(String pathImg, String pathInfo) {
        ...
        try {
            BufferedReader br = new BufferedReader(new FileReader(pathInfo));
        ...
        System.out.println(numPosTest + " " + numNegTest);
        WeakClass[] wk = loadWeakClass(pathWeakClass);

        for (int wkIdx = 0; wkIdx < wk.length; wkIdx++) {
            int cntPos = 0, cntNeg = 0;
            for (int imgIdx = 0; imgIdx < numPosTest + numNegTest; imgIdx++) {
                double sumAlpha = 0., sum = 0.;
                for (int i = 0; i <= wkIdx; i++) {
                    sumAlpha += wk[i].alpha;
                    int idx = wk[i].fi.getIndex();
                ...
                sumAlpha *= 0.5;
                if (sum > sumAlpha && Math.abs(sum - sumAlpha) > 1e-9
                        || Math.abs(sum - sumAlpha) < 1e-9) {
                    if (imgIdx < numPosTest) {
                        cntPos++;
                    }
                } else {
                    if (imgIdx >= numPosTest) {
